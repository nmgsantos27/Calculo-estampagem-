...
function guardarNovoArtigoBD() {
  const nome = document.getElementById('novoNomeArtigo').value.trim();
  const preco = parseFloat(document.getElementById('novoPrecoArtigo').value);

  if (!nome || isNaN(preco) || preco <= 0) {
    alert('Por favor introduza um nome e preço válidos.');
    return;
  }
  ...
  document.getElementById('novoNomeArtigo').value = '';
  document.getElementById('novoPrecoArtigo').value = '';
...
function calcular() {
  ...
  let lucroUn = tipoMargem === 'fixo' ? valMargem : custoTotalUnComIva * (valMargem / 100);
...
function gerarComparativoEscaloes(qtdAtual, custoPecaComIva, portesComIva, tecnica, tipoMargem, valMargem) {
  ...
  listaEscaloes.forEach(q => {
    ...
    let lucroUn = tipoMargem === 'fixo' ? valMargem : custoUn * (valMargem / 100);
