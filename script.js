...
const TECNICAS = {
  dtf: {
    bobina:28,
    obterCusto(qtd, metro, alt, larg) { return custoMetro(qtd, metro, alt, larg, 28); }
  },
  vinil: {
    bobina:50,
    obterCusto(qtd, metro, alt, larg) { return custoMetro(qtd, metro, alt, larg, 50); }
  },
  sublimacao: {
    bobina:58,
    obterCusto(qtd, metro, alt, larg) { return custoMetro(qtd, metro, alt, larg, 58); }
  },
  serigrafia: {
    escaloes:[
...
function calcular(){
  ...
  const elPrecoUn = document.getElementById("resPrecoUn");
  if(elPrecoUn) elPrecoUn.textContent = moeda(venda);
  
  const elTotalCom = document.getElementById("resTotalComercial");
  if(elTotalCom) elTotalCom.textContent = moeda(venda*q);
  
  const elMatUn = document.getElementById("resCustoMaterialUn");
  if(elMatUn) elMatUn.textContent = moeda(pIva);
  
  const elImpUn = document.getElementById("resCustoImprUn");
  if(elImpUn) elImpUn.textContent = moeda(imp.custoUnComIva);
  
  const elPortes = document.getElementById("resPortes");
  if(elPortes) elPortes.textContent = moeda(portesIva);
  
  const elCustoTot = document.getElementById("resCustoTotalLote");
  if(elCustoTot) elCustoTot.textContent = moeda(custoUn*q);
  
  const elLucroUn = document.getElementById("resLucroUn");
  if(elLucroUn) elLucroUn.textContent = moeda(lucro);
  
  const elLucroTot = document.getElementById("resLucroTotal");
  if(elLucroTot) elLucroTot.textContent = moeda(lucro*q);
  
  const elBadge = document.getElementById("escalaoBadge");
  if(elBadge) elBadge.textContent = `Escalão ≥ ${imp.minEscalao} un`;
  
  if(["dtf","vinil","sublimacao"].includes(tecnica)) {
    const elConsumo = document.getElementById("resConsumoFilme");
    if(elConsumo) elConsumo.textContent = `${imp.metrosTotais.toFixed(2).replace(".",",")} m`;
  }
...
function ligarEventos(){
  const registrarAcao = (idv, tipoEvento, fn) => {
    const el = document.getElementById(idv);
    if (!el) return;
    el.addEventListener(tipoEvento, (e) => {
      fn(e);
    }, { passive: true });
  };

  // Suporte universal a cliques/toques para evitar bloqueios no Android
  const clickOuToque = (idv, fn) => {
    const el = document.getElementById(idv);
    if(!el) return;
    el.addEventListener("click", fn);
    el.addEventListener("touchend", (e) => {
      e.preventDefault(); // Previne duplo disparo em alguns emuladores/browsers mobile
      fn(e);
    }, { passive: false });
  };

  registrarAcao("seletorFornecedorBD","change",e=>{fornecedorSelecionadoId=e.target.value;materialSelecionadoId=null;atualizarSelectsDinamicos();});
  registrarAcao("seletorMaterialBD","change",e=>{materialSelecionadoId=e.target.value;const f=baseDados.find(x=>x.id===fornecedorSelecionadoId);atualizarSelectMateriaisDinamicos(f?.materiais||[]);});
  
  clickOuToque("btnNovoForn",()=>abrirFormFornecedor(false));
  clickOuToque("btnEditarForn",()=>abrirFormFornecedor(true));
  clickOuToque("btnEliminarForn",eliminarFornecedor);
  clickOuToque("btnGuardarFornBD",guardarFornecedor);
  clickOuToque("btnFecharFornBD",fecharFormFornecedor);
  
  clickOuToque("btnNovoMat",()=>abrirFormMaterial(false));
  clickOuToque("btnEditarMat",()=>abrirFormMaterial(true));
  clickOuToque("btnEliminarMat",eliminarMaterial);
  clickOuToque("btnGuardarMatBD",guardarMaterial);
  clickOuToque("btnFecharMatBD",fecharFormMaterial);
  
  registrarAcao("tecnica","change",alternarTecnica);
  
  ["quantidade","custoPeca","portesFornecedor","custoMetroDTF","alturaEstampaDTF","larguraEstampaDTF","numCores","tipoMargem","valMargem"]
    .forEach(x=>registrarAcao(x,"input",calcular));
    
  registrarAcao("tipoMargem","change",calcular);
  registrarAcao("numCores","change",calcular);
  
  clickOuToque("btnCopiarResumo",copiarResumo);
  clickOuToque("btnGuardarPDF",guardarPDF);
  clickOuToque("btnImprimir",imprimir);
}
